...
echo [3/5] Get-IisApplicationVersion ^(ms- servers uit servers.txt^)...
powershell -ExecutionPolicy Bypass -Command "$servers = (Get-Content -Path '%SERVERS_FILE%').Where({$_.Trim() -ne '' -and $_ -notmatch '^#' -and $_ -like 'ms-*'}); Write-Host ('  IIS servers (ms-): ' + ($servers -join ', ')); & '%~dp0Get-IisApplicationVersion.ps1.txt' -ComputerName $servers"
echo.
